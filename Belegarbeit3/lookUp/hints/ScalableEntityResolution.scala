...
class ScalableEntityResolution(sc:SparkContext, dat1:String, dat2:String, stopwordsFile:String, goldStandardFile:String)  extends EntityResolution(sc,dat1,dat2,stopwordsFile,goldStandardFile) {  
  ...
  def buildInverseIndex:Unit={
    ...
    this.amazonInvPairsRDD = amazonWeightsRDD_.flatMap(X=>ScalableEntityResolution.invert(X)).cache
    this.googleInvPairsRDD = googleWeightsRDD_.flatMap(X=>ScalableEntityResolution.invert(X)).cache
  }
  ...
  def determineCommonTokens:Unit={
    //val amazonInvPairsRDD_ = this.amazonInvPairsRDD
    //val googleInvPairsRDD_ = this.googleInvPairsRDD
    this.commonTokens = this.amazonInvPairsRDD.join(this.googleInvPairsRDD).map(X=>ScalableEntityResolution.swap(X)).reduceByKey((X:String,Y:String)=>(X+" "+Y)).map(X=>(X._1,X._2.split(" ")))
  }
  ...
  def swap(el:(String,(String,String))):((String, String),String)={
    ...
    */
    def fastCosinusSimilarity(record:((String,String),Iterable[String]), 
      ...
      val amazonWeights = amazonWeightsBroad.value
      val amazonNorms = amazonNormsBroad.value
      val googleWeights = googleWeightsBroad.value
      val googleNorms = googleNormsBroad.value
      ???
  }
